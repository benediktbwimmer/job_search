...
def _fake_llm_eval(job, profile, constraints, model, description_max_chars=2500, input_description_max_chars=20000):
    ...
    score = 84 if "Senior" in title else 66
...
class PipelineCompatibilityTests(unittest.TestCase):
    def test_pipeline_outputs_are_ranked_via_llm_stage(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                ...
                patch("job_search.pipeline.DB", db_dir),
                ...
                patch("job_search.pipeline.uuid.uuid4", return_value=UUID("00000000-0000-0000-0000-000000000001")),
                patch("job_search.pipeline.persist_run_metadata", return_value=[]),
                patch("builtins.print"),
            ):
                summary = run_pipeline()

            actual_summary = json.loads((output_dir / "latest_report.json").read_text())
            actual_jobs = json.loads((data_dir / "jobs_normalized.json").read_text())

            self.assertEqual(summary["total"], 2)
            self.assertEqual(actual_summary["tiers"]["A"], 1)
            self.assertEqual(actual_summary["tiers"]["B"], 1)
            self.assertTrue(all(x.get("scored_by") for x in actual_jobs))
            self.assertTrue(all("llm_summary" in x for x in actual_jobs))
